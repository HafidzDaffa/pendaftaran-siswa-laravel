...
@section('content')
    <div class="w-full flex flex-col py-[40px] px-[55px] gap-y-[80px] text-[24px]">
        <form action="{{ route('user.store') }}" method="post" class="px-[48px] py-[66px] flex flex-col gap-y-[108px] items-center">
            @csrf
            <div class="w-full flex flex-col rounded-lg">
                ...
                <div class="grid grid-cols-6 py-[54px]">
                    ...
                    <div class="flex flex-col gap-y-[32px] col-span-3">
                        <input type="text" id="nama" name="nama" value="{{ old('nama') }}" class="form-input rounded-lg p-2 border border-black w-full">
                        @error('nama')
                        <span class="text-red-500">{{ $message }}</span>
                        @enderror
                        <input type="number" id="no_wa" name="no_wa" value="{{ old('no_wa') }}" class="form-input rounded-lg p-2 border border-black w-full">
                        @error('no_wa')
                        <span class="text-red-500">{{ $message }}</span>
                        @enderror
                        <input type="email" id="email" name="email" value="{{ old('email') }}" class=" form-input rounded-lg p-2 border border-black w-full">
                        @error('email')
                        <span class="text-red-500">{{ $message }}</span>
                        @enderror
